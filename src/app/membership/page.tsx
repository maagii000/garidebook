  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10">
      <Reveal className="text-center max-w-2xl mx-auto mb-10">
        ...
        <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-black">Өөрт тохирох багцаа сонгоно уу.</h1>
        <p className="text-slate-500 text-lg mt-3">Сурагчдад зориулсан мэдлэг, солилцоо, хөгжлийн нэгдсэн систем.</p>
        <div className="mt-5 inline-flex bg-slate-100 p-1 rounded-full text-sm font-medium">
          <span className="px-4 py-2 rounded-full bg-white shadow-sm text-black">
            Одоогийн төлөв: <span className="font-extrabold text-brand">{plan ?? "Багцгүй"}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
        ...
        <Reveal delay={80} className="bg-white rounded-[2rem] p-8 shadow-apple border border-gray-100 flex flex-col relative overflow-hidden">
          ...
          <h3 className="text-2xl font-extrabold mb-2 text-black">Base</h3>
          ...
          <ul className="space-y-4 mb-8 flex-grow">
            {BASE_PERKS.map((f) => (
              <li key={f.text} className={`flex items-center gap-3 ${f.ok ? "" : "text-gray-400"}`}>
                {f.ok ? <CheckIcon className="text-emerald-500" /> : <XIcon />}
                {f.text}
              </li>
            ))}
          </ul>
          {plan === "BASE" ? (
            <div className="w-full bg-slate-100 text-slate-400 py-4 rounded-full font-bold text-center">Идэвхтэй байна</div>
          ) : firstTimer ? (
        ...
        <Reveal delay={160} className="bg-black text-white rounded-[2rem] p-8 shadow-apple-hover border border-gray-800 flex flex-col relative">
          <div className="absolute top-0 right-0 bg-gradient-to-r from-brand to-blue-400 text-white text-xs font-bold px-4 py-1 rounded-bl-xl">
            Хамгийн эрэлттэй
          </div>
          ...
          <div className="mb-8">
            <span className="text-4xl font-extrabold">₮{price("PRO")}</span>
          ...
          {plan === "PRO" ? (
            ...
            <button onClick={() => claimTrial("PRO")} disabled={busy}
              className="w-full bg-brand text-white py-4 rounded-full font-bold hover:bg-brand-dark transition-colors disabled:opacity-50">
              {busy ? "Идэвхжүүлж байна..." : "Үнэгүй эхлэх"}
            </button>
          ) : (
            <button
              onClick={() => { if (!session) { router.push("/login"); return; } setPayFor("PRO"); }}
              className="w-full bg-brand text-white py-4 rounded-full font-bold hover:bg-brand-dark transition-colors">
              Pro эрх авах
            </button>
          )}
        </Reveal>
      ...
      {!session && (
        <p className="mt-6 text-center text-sm text-slate-500">
          Багц авахын тулд эхлээд <button onClick={() => router.push("/login")} className="font-bold text-brand hover:underline">нэвтэрнэ үү</button>.
        </p>
      )}
