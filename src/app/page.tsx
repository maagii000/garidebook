...
export default function Home() {
  ...
  return (
    <div className="pb-10">
      {/* HERO — цайвар + navy калькулятор карт */}
      <section className="bg-white border-b border-slate-100 overflow-hidden">
        <div className="mx-auto max-w-7xl px-4 py-10 md:py-14 grid gap-8 md:grid-cols-[1.1fr_0.9fr] items-center">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-emerald-50 border border-emerald-100 px-3 py-1.5 text-xs font-bold text-emerald-700">
              <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
              UFE Entrepreneurship • Prototype / MVP
            </div>
            <h1 className="mt-4 text-3xl md:text-[44px] font-extrabold leading-[1.15] text-navy">
              Уншсан номоо оруул,
              <br />
            ...
            <p className="mt-4 text-slate-500 leading-7 max-w-md text-[15px]">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link
                href="/books/new"
                className="rounded-full bg-navy px-6 py-3 text-sm font-extrabold text-white hover:bg-navy-dark transition shadow-lg shadow-navy/20"
              >
                Ном оруулах
              </Link>
              <Link
                href="/catalog"
                className="rounded-full bg-accent px-6 py-3 text-sm font-extrabold text-white hover:bg-accent-dark transition shadow-lg shadow-orange-900/20"
              >
                Каталоги
              </Link>
            </div>
            <div className="mt-6 flex gap-8 text-sm">
              <div>
                <div className="text-2xl font-extrabold text-navy">{books.length}+</div>
                <div className="text-slate-400 text-xs mt-0.5">ном</div>
              </div>
              <div>
              ...
              <div>
                <div className="text-2xl font-extrabold text-navy">5,000₮</div>
                <div className="text-slate-400 text-xs mt-0.5">суурь үнэ</div>
              </div>
            </div>
          </div>

          {/* Кредит калькулятор card — navy */}
          <div className="rounded-3xl bg-navy text-white p-6 shadow-2xl shadow-navy/30">
            <div className="text-xs font-extrabold uppercase tracking-wide text-white/50">
              Кредит калькулятор
            </div>
            <div className="mt-1 text-lg font-extrabold">
              {calcCredit} кредит = <span className="text-accent">−{calcDiscount.toLocaleString()}₮</span>
            </div>
            <input
              type="range"
              min={0}
            ...
            />
            <div className="mt-1 flex justify-between text-[11px] text-white/50 font-bold">
              <span>0</span>
            ...
            <div className="mt-4 rounded-2xl bg-white/10 p-4 flex items-center justify-between">
              <div>
                <div className="text-xs text-white/50 line-through">5,000₮</div>
              ...
              <Link
                href="/catalog"
                className="rounded-full bg-accent px-4 py-2.5 text-xs font-extrabold text-white hover:bg-accent-dark"
              >
                Энэ үнээр авах →
              </Link>
            </div>
            <div className="mt-4 space-y-2 text-[13px]">
              {[
                ...
                <div key={n} className="flex items-center gap-3 rounded-xl bg-white/5 border border-white/10 px-3 py-2">
                  ...
                  <span className="ml-auto text-xs font-bold text-emerald-400">{d}</span>
                </div>
              ))}
            </div>
          </div>
